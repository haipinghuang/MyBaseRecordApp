...
//            //根据需求来。
    ...
    private ImageSize getImageViewSize(ImageView imageView) {
        ...
        if (width <= 0) {
            width = getImageFiledValue(imageView, "mMaxWidth");
        }
        if (width <= 0) {// 取成屏幕的值
        ...
        if (height <= 0) {
            height = getImageFiledValue(imageView, "mMaxHeight"); //通过反射解决 只有api 16才能调用的方法getMaxHeight
        }
        if (height <= 0) {// 取成屏幕的值
    ...
    private synchronized void addTask(Runnable runnable) {
        ...
        try {
            if (mPoolThreadHandler == null) {
                //判断mPoolThreadHandler是否已经初始化，否则阻塞
